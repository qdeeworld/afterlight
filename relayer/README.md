...
- `/health` exposes collapsed exit and funding states inside `claimCapacity`. Both the browser and the neutral checkpoint route refuse new funding unless the observed live neutral balance and exact `12 STRK` allowance can cover one bounded claim or cancellation, the health floor, the exit ledger has no active reservation or same-day spend, sponsorship is unfrozen, and the contract has zero outstanding liability. The checkpoint route performs this fresh read immediately before the sponsored checkpoint, closing stale-tab admission. The release is operated one reserve at a time: admission pauses as soon as a liability is observed. This is a fail-closed operational admission rule, not a contract-level proof that two simultaneous zero-liability observations cannot race. A successful exit consumes `6 STRK` of allowance and exhausts capacity until a reviewed replenishment on a later UTC day.
