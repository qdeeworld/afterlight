...
- `/health` exposes collapsed exit and funding states inside `claimCapacity`. The browser and neutral checkpoint route derive bounded exit slots from fee-aligned pool allowance, the live sponsor balance, the `1 STRK` retained floor, the `7.5 STRK` maximum network-fee reservation, and the configured three-vault ceiling. New funding remains available only while the current `1 STRK` liabilities consume fewer slots than those conservatively backed exits, no exit reservation or submission owns the shared nonce lane, sponsorship is unfrozen, the `22.5 STRK` daily exit budget can reserve another bounded exit, and no funding-admission lease is active. The checkpoint route performs this fresh read and atomically acquires the lease immediately before sponsorship, closing stale-tab and concurrent ordinary-client admission. A direct caller can still use the deployed contract's permissionless checkpoint and create another fully collateralized liability outside this supported route; this cannot consume another vault's backing, but its sponsored exit may queue until capacity is restored. Each successful exit consumes exactly `6 STRK` of allowance, after which health recomputes the remaining backed capacity instead of assuming the service is globally exhausted.
