...
- Before broadcasting an exit, the relayer atomically stores the deterministic outer transaction hash and exact signed transaction on the still-releasable reservation. A definitive RPC rejection releases and deletes that artifact; an accepted or ambiguous broadcast reconciles the exact stored hash and only then transitions it to `SUBMITTED`. Terminal reconciliation deletes the artifact. It contains no signing key and must never be logged, exported, or retained after reconciliation.
