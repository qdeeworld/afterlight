...
- Before broadcasting an exit, the relayer stores the deterministic outer transaction hash on the still-releasable reservation. A definitive RPC rejection releases that pre-broadcast record; an accepted or ambiguous broadcast reconciles the exact stored hash and only then transitions it to `SUBMITTED`. No post-broadcast ledger write is required to recover the expected hash.
...
- `/health.claimCapacity`; stop new reserves whenever `fundingStatus` is not `ready`. The browser checks this state and the checkpoint route freshly rechecks it immediately before sponsorship. The exact ready allowance is `12 STRK`, covering one claim or cancellation, and funding is ready only when observed total locked liability is zero, sponsorship is unfrozen, neither control nor exit has an active shared-nonce reservation, and the exit ledger can still reserve the full `7.5 STRK` ceiling that UTC day. This is not an atomic contract-level serialization primitive, so monitor for simultaneous zero-liability admission attempts and disable submission if one is detected. After either exit, the remaining `6 STRK` allowance and same-day ledger spend intentionally exhaust capacity until a newly reviewed replenishment on a later UTC day.
