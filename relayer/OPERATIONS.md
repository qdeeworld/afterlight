...
- Before broadcasting any control or exit transaction, the relayer atomically stores the deterministic outer transaction hash and exact signed transaction on the still-releasable reservation. An exact retry revalidates and rebroadcasts only that persisted artifact. A hashless `RESERVED` row is provably pre-broadcast and may be released by the exact retry; a concurrent original request must observe a failed prepare transition and cannot proceed to broadcast. A definitive RPC rejection releases and deletes a prepared exit artifact; an accepted or ambiguous broadcast reconciles the exact stored hash and only then transitions it to `SUBMITTED`. Terminal reconciliation deletes the artifact. It contains no signing key and must never be logged, exported, or retained after reconciliation.
